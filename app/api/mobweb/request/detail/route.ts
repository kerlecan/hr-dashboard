...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body?.persid || !body?.requestId) {
      return NextResponse.json(
        { 
          success: false, 
          message: "persid ve requestId zorunludur" 
        },
        { status: 400 }
      )
    }
    ...
    /* --------------------------------------------------
     * MOBWEB REQUEST DETAIL API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobweb/request/detail`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Talep detay servisi hata verdi"
      ...
      let message = "Talep detay API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Talep detay API zaman aşımı"
      }
    ...
    console.error("MobWeb Request Detail Route Error:", error)
    return NextResponse.json(
      {
        success: false,
