...
export async function POST(request: NextRequest) {
  try {
    ...
    /* --------------------------------------------------
     * MOBWEB REQUEST MY API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobweb/request/my`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Taleplerim servisi hata verdi"
      ...
      let message = "Taleplerim API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Taleplerim API zaman aşımı"
      }
    ...
    console.error("MobWeb Request My Route Error:", error)
    return NextResponse.json(
      {
        success: false,
