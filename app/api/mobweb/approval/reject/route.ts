...
export async function POST(request: NextRequest) {
  try {
    ...
    /* --------------------------------------------------
     * MOBWEB APPROVAL REJECT API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobweb/approval/reject`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Reddetme servisi hata verdi"
      ...
      let message = "Reddetme API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Reddetme API zaman aşımı"
      }
    ...
    console.error("MobWeb Approval Reject Route Error:", error)
    return NextResponse.json(
      {
        success: false,
