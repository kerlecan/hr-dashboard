...
export async function POST(request: NextRequest) {
  try {
    ...
    /* --------------------------------------------------
     * MOBWEB APPROVAL PENDING API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobweb/approval/pending`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Onay bekleyenler servisi hata verdi"
      ...
      return NextResponse.json({
        success: true,
        source: "mobweb-approval-pending",
      ...
      let message = "Onay bekleyenler API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Onay bekleyenler API zaman aşımı"
      }
    ...
    console.error("MobWeb Approval Pending Route Error:", error)
    return NextResponse.json(
      {
        success: false,
