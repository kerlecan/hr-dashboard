...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body?.persid || !body?.approvalId) {
      return NextResponse.json(
        { 
          success: false, 
          message: "persid ve approvalId zorunludur" 
        },
        { status: 400 }
      )
    }
    ...
    /* --------------------------------------------------
     * MOBWEB APPROVAL APPROVE API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobweb/approval/approve`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Onaylama servisi hata verdi"
      ...
      return NextResponse.json({
        success: true,
        source: "mobweb-approval-approve",
      ...
      let message = "Onaylama API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Onaylama API zaman aşımı"
      }
    ...
    console.error("MobWeb Approval Approve Route Error:", error)
    return NextResponse.json(
      {
        success: false,
