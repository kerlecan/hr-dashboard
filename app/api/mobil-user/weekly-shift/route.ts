...
/**
 * Database adlarına göre Node-RED URL mapping
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body?.persid) {
      return NextResponse.json(
        { success: false, message: "persid zorunludur" },
        { status: 400 }
      )
    }

    /* --------------------------------------------------
     * NODE-RED WEEKLY SHIFT API
     * -------------------------------------------------- */
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobil-user/weekly-shift`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let message = "Shift servisi hata verdi"
        if (res.status === 401) message = "Yetkisiz erişim"
        else if (res.status === 404) message = "Shift endpoint bulunamadı"
        else if (res.status >= 500) message = "Shift servis sunucu hatası"
      ...
      let message = "Shift API erişilemiyor"
      if (err.name === "AbortError") {
        message = "Shift API zaman aşımı"
      }
    ...
    console.error("Weekly Shift Route Error:", error)
    return NextResponse.json(
      {
        success: false,
