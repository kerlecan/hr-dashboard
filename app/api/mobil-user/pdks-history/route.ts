...
export async function POST(request: NextRequest) {
  try {
    ...
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobil-user/pdks-history`
    ...
    try {
      ...
      clearTimeout(timeoutId)
      let errorMessage = "Pdks History API'ye bağlanılamadı"
      if (fetchError.name === "AbortError") errorMessage = "Pdks History API zaman aşımı"
    ...
    console.error("Pdks History  API error:", error)
    return NextResponse.json(
