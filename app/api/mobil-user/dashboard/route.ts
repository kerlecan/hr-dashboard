...
export async function POST(request: NextRequest) {
  try {
    ...
    const API_URL = `${apiBaseUrl}/butunbiApi/api/mobil-user/dashboard`
    ...
    try {
      ...
      clearTimeout(timeoutId)
      let errorMessage = "Mobil User Dashboard API'ye bağlanılamadı"
      if (fetchError.name === "AbortError") errorMessage = "Mobil User Dashboard API zaman aşımı"
    ...
    console.error("Mobil User Dashboard API error:", error)
    return NextResponse.json(
